...
for (let i = 0; i < starWarsCharacters.length; i++) {
  let firstone = starWarsCharacters[i];
  if (firstone.gender == "female") {
    delete firstone.height;
    delete firstone.birth_year;
    delete firstone.skin_color;
    femaleCharacters.push(firstone);
  }
}
...
console.log(" blue is array; ", Array.isArray(eyeColor.blue));
/* ESERCIZIO 5
...
for (let i = 0; i < starWarsCharacters.length; i++) {
  let coloORreOcchi = starWarsCharacters[i].eye_color;

  switch (coloORreOcchi) {
    case "blue":
    ...
    case "blue-gray":
      ...
      default:
        console.log("color occhi e brown");
  }
}
...
while (i < starWarsCharacters.length) {
  ...
  massatotale = massatotale + parseInt(mass);
  i++;
}
...
switch (massatotale > 500 || massatotale < 500) {
